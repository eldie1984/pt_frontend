...
import { getInstruments, getInstrumentBySymbol, getExchanges, createInstrument, createTransaction, getTransactions, type Exchange } from "@/lib/api/instruments"
...
export function InstrumentsScreen() {
  ...
  const fetchTransactions = useCallback(async (portfolioId: number) => {
    if (!token) return
    try {
  ...
  const fetchHoldings = useCallback(async () => {
    ...
    setError(null)
    try {
      const portfoliosRes = await fetch(`${API_BASE}/api/portfolio/portfolios/`, {
        headers: authHeaders(),
      })
      ...
      const portfolioRes = await fetch(`${API_BASE}/api/portfolio/portfolios/${portfolios[0].id}`, {
        headers: authHeaders(),
      })
  ...
  const fetchExchanges = useCallback(async () => {
    ...
    try {
      ...
      // Don't show error for exchanges, just use fallback
    }
  }, [token])
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Get the first portfolio ID (simplified - in real app you'd let user choose)
      const portfoliosRes = await fetch(`${API_BASE}/api/portfolio/portfolios/`, {
        headers: authHeaders(),
      })
      if (!portfoliosRes.ok) throw new Error("Failed to fetch portfolios")
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                ...
                >
                  ...
                  {/* Fallback to hardcoded exchanges if API fails */}
                  {exchanges.length === 0 && STOCK_MARKETS.map((m) => (
